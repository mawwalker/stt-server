...
public:
  ...
  // 为每个会话创建独立的VAD实例
  std::unique_ptr<sherpa_onnx::cxx::VoiceActivityDetector>
  create_vad_instance() const;
...
public:
  ...
  size_t get_active_instances() const {
    return total_instances.load() - available_instances.load();
  }
};
...
public:
  ...
  std::string recognize_with_metadata(const float *samples, size_t sample_count,
                                      std::string &language,
                                      std::string &emotion, std::string &event,
